...
function attemptWordSelection(words: string[], targetCount: number): { success: boolean; words: string[] } {
  ...
  while (selectedWords.length < targetCount) {
    ...
    while (attemptedIndices.size < words.length) {
      ...
      const validation = validateWordForGrid(candidateWord, selectedWords, usedLetters, alphabet, targetCount);
...
function validateWordForGrid(
  ...
  alphabet: string,
  targetCount: number = WORDS_TO_FIND_COUNT
): WordValidation {
  ...
  // Check if adding this word would use all letters (only enforce for larger word sets)
  if (targetCount >= 8) {
